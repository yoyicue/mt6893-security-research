...
The current result is **reachable through APUSYS memory import, normal-VPU algorithm lookup, command parsing, scheduler handoff, exact-size VPU request acceptance, and APUNN settings/output completion from `system_app`**. The VPU request ABI is now tied back to the target wrapper's preferred `/system/lib64/libvpu5.so`: `request+0x35` is `buffer_count`, `request+0x38/+0x40` are settings length/IOVA when the libvpu property path is used, `request+0x50` starts the per-buffer descriptor array, and the outer APUSYS subcommand `cb_info_size` must be exactly `0xb70` for this provider path. Runtime proves the midware set/clear copyback mechanism, descriptor-0 state writeback when descriptor slot 0 points at code/output, and the normal wrapper completion condition when all five native descriptors point at the wrapper DSP command/settings buffer. In that `settings5/no-settings` shape, APUNN changes settings flags `0x5 -> 0x7`, which satisfies the host predicate `(settings[0] & 0x0a) == 0x02`, clears the standard one-entry data-descriptor pointer at settings `+0x30`, and writes the output section while `mdw_usr_wait_cmd` returns `0`. Completed-shape opcode, operand-id, operand-offset, operation-count, code-size, output-size, data-descriptor, data-entry, descriptor-target, and data-payload matrices now show `10001..10009`, output operand ids `0/1/2/3/0xffff`, tested operand-list offsets including boundary/OOB placement, tested input/output count combinations, tested code sizes, tested output sizes, tested data descriptor flags/orderings, tested descriptor targets, and tested data-payload word patterns. `settings+0x04` is a real code-section acceptance gate: code sizes `0/4/8/0xc/0x10` fail with `-EIO`, `0x11` is the smallest tested completed size, and clean/success-only reruns show `0x11/0x14/0x1c/0x20/0x40/0x48/0x4c/0x1c9` complete. After a batch prefix of failing low-size requests, later valid sizes can still return wait success without APUNN settings/output completion, so settings/output memory is the completion oracle. `settings+0x08` bounds the maximum output fill; `data_desc_size=0x0c` is the observed standard one-entry consumption size; two-entry descriptors keep `settings+0x30` nonzero, and flags cleanup is target/order-sensitive. Tested settings/code/output/data/plane targets and data payload patterns do not produce source-sensitive output. This is APUNN output completion, but not an information leak or timeout lifetime misuse. The direct ioctl probes now separate the libvpu property path from the ordinary target `XrpVpuStream::CreateVpuRequest()` path: `VpuStreamLibManager` resolves `stream+0x88` to `vpuRequest_addBuffer` and `stream+0x90` to `vpuRequest_setProperty`, and `CreateVpuRequest()` uses five `addBuffer()` calls with the same DSP command/settings buffer descriptor, leaving `setProperty()` unused. Embedded MVPU RISC-V kernels in `libmvpuop_mtk_nn.mtk.so` expose ordered `.mvpu.kernel.info` argument type signatures, but they do not decode the `apu_lib_apunn` XRP command parser; see [`docs/APUNN_SETTINGS_ABI.md`](docs/APUNN_SETTINGS_ABI.md). IDA analysis shows the device is the MTK APUSYS midware character device. Its main ioctl handler is now named `mdw_ioctl` in the IDB at `0xffffffc00878a0ec`.
...
The current allocator-control target is documented in [`docs/ALLOCATOR_CONTROLLABILITY_OPPORTUNITY.md`](docs/ALLOCATOR_CONTROLLABILITY_OPPORTUNITY.md). It tracks the `target_then_lower` exact IOVA reuse opportunity, the metrics required to make that opportunity controllable, and the firmware re-entry criteria.

FLIX128 boundary byte analysis of the APUNN firmware (`0x70044b74`, the top iDMA orchestration function, 3880 bytes) confirms that INPUT DMA is synchronous: `iDMA_schedule` and `iDMA_wait` error-handler L32Rs sit in consecutive FLIX128 bundles with only a 9-byte gap. Output write is most likely performed by CPU-store instructions inside the 20 KB compute kernel `0x700a21b8` (called 4 times), not by a separate iDMA pass. The dmaif utility `0x70044850` (804 bytes, zero `.rodata` L32R refs) is called after the compute passes and is likely a DMA barrier/flush. This closes Q1 for the current `XTENSA_ANN_VERSION` PoC shape: the firmware write path is fully synchronous from the Java-layer perspective, and the `free_after=0ms` completed race is always lost. The new highest-priority untested combination is **descriptor-plane redirect + target/lower IOVA reuse**: the plane-redirect path produces a ~9 s timeout firmware write, far slower than the Java `mem_free` round-trip, making timing-based cross-buffer write plausible if exact IOVA reuse occurs. See `docs/CONTROLLED_OPPORTUNITIES.md` §6 for the proposed probe shape.

This directory documents the ioctl surface and current runtime probes. The Java probe covers reject/query paths, negative memory-create cases, optional device-control reachability checks, direct dmabuf-source checks, a candidate-fd scan for the memory import path, HardwareBuffer-backed dmabuf import, controlled `ucmd` gate tests, zero-header / invalid-subcommand `run_cmd_async` parser probes, a normal-VPU valid-type request-size guard probe, a full-size (`0xb70`) VPU execution probe, a chained IOVA-import + VPU request probe, an XRP-shaped APUNN settings probe, no-dispatch IOVA controls, a small APUNN/XRP opcode/operand matrix mode, and two-native-buffer internal-command-shaped modes with minimal/libvpu-style descriptor metadata, wrapper send-state command flags, output-first descriptor order, wrapper-sized settings, output-ready header state, one standard APUNN data descriptor, an Xtensa operation operand-list offset matrix, target-wrapper-shaped five-code-descriptor/no-settings-property diagnostics, and the closer five-settings-descriptor/no-settings-property replay that now reaches APUNN completion. The completed settings-backed replay now includes opcode, output-operand-id, operand-list offset, operation input/output-count, code-size, output-size/header, data-descriptor size/pointer, explicit data-entry flags/order, descriptor target-section, and data-payload pattern matrices. Timeout/teardown has a first close-race pass: residual in-flight command cleanup is reachable, but no UAF/crash is demonstrated. The remaining closure work is to map richer output/data source semantics in the completed settings-backed shape and, if needed, exercise the real service-wrapper request path rather than broad firmware parameter matrices.

## Docs

- [`docs/APUNN_SETTINGS_ABI.md`](docs/APUNN_SETTINGS_ABI.md)
- [`docs/ALLOCATOR_CONTROLLABILITY_OPPORTUNITY.md`](docs/ALLOCATOR_CONTROLLABILITY_OPPORTUNITY.md)
- [`docs/CONTROLLED_OPPORTUNITIES.md`](docs/CONTROLLED_OPPORTUNITIES.md)
- [`docs/HANDOFF_FIRMWARE_DISASSEMBLY.md`](docs/HANDOFF_FIRMWARE_DISASSEMBLY.md)
- [`docs/HANDOFF_KERNEL_PRIMITIVE.md`](docs/HANDOFF_KERNEL_PRIMITIVE.md)
- [`docs/MTK_SOURCE_FIRMWARE_COMPARISON_MATRIX.md`](docs/MTK_SOURCE_FIRMWARE_COMPARISON_MATRIX.md)
- [`docs/MTK_SOURCE_FIRMWARE_COMPARISON_PLAN.md`](docs/MTK_SOURCE_FIRMWARE_COMPARISON_PLAN.md)
...
| `XTENSA_INFO11` | executed slot value for preload / D2D_EXT |
...
| D2D copy helper | `0xffffffc0087a20e8` / `0xffffffc0087a20f8` | Copies `request+0x50` into the clamped-slot VPU command buffer |
| D2D array IOVA helper | `0xffffffc0087a20c8` | Returns the copied descriptor-array IOVA from `core + slot * 0xb0 + 0x400` |
| D2D_EXT preload selector | `0xffffffc0087a1f40` | Clamps slot to `0..2` and loads the selected Preload object from `core + slot * 0xb0 + 0x3c8` |
| D2D wait helper | `0xffffffc0087a6374` | Waits on `core + slot * 0xb0 + 0x3d0` without clamping; normal provider paths feed slot `0` or an internal allocator slot |
| D2D status mapper | `0xffffffc0087a2040` | Maps firmware status `1/2` to success, `0/8` to `-EIO`, `3/5..7/9..15` to `-EINVAL`, `4` to `-EBUSY`, and `16/0xff` to `-EBADFD` |
...
the selected Preload object contributes additional firmware inputs. The source
mirror names them as `XTENSA_INFO16 = pre->a.mva + pre->a.entry_off` and
`XTENSA_INFO19 = pre->a.iram_mva`, derived from packed `struct vpu_pre_info`
metadata (`name`, `off`, `pAddr`, `file_sz`, `flag`, `info`, `start_addr`)
before the common buffer/settings tuple.
...
The target-wrapper/no-settings diagnostic
`--run-cmd-vpu-xrp-target-code5-no-settings-iova` keeps the five copied
code/input descriptors but clears the native VPU settings tuple
(`request+0x38 = 0`, `request+0x40 = 0`) because the target
`CreateVpuRequest()` path leaves the resolved `vpuRequest_setProperty()` slot
unused. Its control leaves all windows unchanged. Dispatch returns `0`, VPU
boot/map logs are present, settings/output/data windows remain unchanged, and
code word `0` changes `0x2713 -> 0x271b`. The explicit wait variant
...
settings completion or output writeback. The 2026-06-15 summary rerun adds
request-field dumps: before and after dispatch, `result_status=0`,
`slot_b68=0`, and `algo_ret_b6c=0`, while code word `0` still changes
`0x2713 -> 0x271b`. This shape is useful for descriptor-0 status attribution,
but it is not the closest replay of the target wrapper because those five
descriptors point at code/input instead of the wrapper DSP command/settings
buffer.

The closer target-wrapper replay is
`--run-cmd-vpu-xrp-target-settings5-no-settings-wait-iova`. It clears
`request+0x38/+0x40` like the ordinary target path, sets `buffer_count=5`, and
points all five libvpu-style native descriptors at the wrapper DSP
command/settings buffer at offset `0` with size `0x68`. The fixed-log rerun
prints `plane0_mva` at the settings IOVA, dispatch returns `0`, and
`mdw_usr_wait_cmd` returns `0`. Compared with the no-dispatch control, firmware
changes settings flags `0x5 -> 0x7`, clears the standard APUNN data descriptor
pointer at settings `+0x30`, leaves the code/input operation window unchanged,
and writes the output section words through offset `0x40` to `0xffffffff`.
This is the normal APUNN settings/output completion contract for the tested
`XTENSA_ANN_VERSION` request shape. It is not a leak primitive: the copied
output is fixed completion data, and the control run leaves settings/output
unchanged.
...
that settings-backed request for recovered opcodes `10001..10009`. Its control
keeps settings at `0x5`, keeps settings `+0x30` pointing at the data descriptor,
and preserves the initialized output header for every opcode. Dispatch plus
wait returns `0` for every tested opcode, changes settings to `0x7`, clears
settings `+0x30`, leaves the code/opcode window unchanged, and writes
deterministic output. Opcodes `10001`, `10002`, `10003`, `10005`, `10006`,
`10007`, `10008`, and `10009` fill output through offset `0x40`; `10004`
fills through offset `0x3c` and leaves offset `0x40` at zero. The earlier
code-first `10005..10007` timeout/error behavior is therefore an incomplete
descriptor-target behavior, not the completed settings-backed opcode behavior.
...
keeps the same settings-backed request, keeps `XTENSA_ANN_VERSION`, and varies
the single output operand id through `0`, `1`, `2`, `3`, and `0xffff`. Its
control preserves the requested operand id, settings `0x5`, settings `+0x30`,
and the initialized output/data windows. Dispatch plus wait returns `0` for
every tested operand id, changes settings to `0x7`, clears settings `+0x30`,
and leaves the code/operand-list words unchanged. Data descriptor and data
payload windows remain unchanged. The output tail is not a stable function of
the operand id: one run wrote through offset `0x3c` for id `3`, while the
repeat wrote through `0x3c` for ids `1` and `2` and through `0x40` for id `3`.
The operand id therefore does not gate completion, and it is not yet a stable
label for output semantics.
...
keeps the same settings-backed request, keeps opcode `XTENSA_ANN_VERSION`,
uses one output operand id `0`, and varies the operand-list offset through
`0`, `0x10`, `0x40`, `0x100`, `0x17e`, and `0x180`. Offset `0x17e` places the
single two-byte operand at the end of the advertised `0x1c8` operation entry;
`0x180` places it just outside that entry and before the output section. The
control preserves the requested offset words and initialized windows. Dispatch
and repeat both return `0` from submit and wait for every case, change settings
to `0x7`, clear settings `+0x30`, leave the code window unchanged, and write the
same completion-style output through offset `0x40`. This offset field is
therefore not a completion gate for this opcode/oracle, and the tested OOB
offset is not rejected. This does not yet prove source-sensitive consumption of
the operand list.
...
same settings-backed request, keeps opcode `XTENSA_ANN_VERSION`, and varies the
operation shape across `0/0`, `0/1 [0]`, `0/2 [0,1]`, `1/0 [0]`,
`1/1 [0,1]`, and `2/1 [0,1,2]`. Its control preserves every requested
input/output count and operand-list word. Dispatch plus wait returns `0` for
every tested shape, changes settings to `0x7`, clears settings `+0x30`, and
leaves the code window unchanged. Data descriptor and data payload windows
remain unchanged. The output tail is again not stable enough to assign count
semantics: the first run wrote only the header word for `0/1 [0]`, while the
repeat wrote only the header word for `0/2 [0,1]`; the other observed cases
wrote through offset `0x40`. The tested input/output counts therefore do not
gate completion; output-size and data-source semantics remain separate work.
...
five-descriptor/no-settings request, then vary `settings+0x04`. The original
matrix covered `0`, `4`, `0x48`, `0x1c7`, `0x1c8`, `0x1c9`, and `0x390`. The
range matrix adds `8/0xc/0x10/0x11/0x12/0x13/0x14/0x18/0x1c/0x20`,
`0x3f/0x40/0x44/0x47/0x49/0x4c`, and `0x1ca/0x1cc`; the clean matrix reruns only
accepted-looking sizes without a failing low-size prefix. Controls preserve every
initialized code-size value and skip only dispatch.

Dispatch plus wait consistently fails for code sizes `0`, `4`, `8`, `0xc`, and
`0x10` with `mdw_usr_wait_cmd = -EIO`, settings left at `0x5`,
`settings+0x30` preserved, output header unchanged, and native
`result_status=0x2`. Code size `0x11` is the smallest tested value that reaches
the normal APUNN completion predicate: settings become `0x7`, `settings+0x30` is
cleared, and output is filled. In the clean/success-only runs, every selected
valid size `0x11/0x14/0x1c/0x20/0x40/0x48/0x4c/0x1c9` completes in both
dispatch runs. The full range runs also expose batch state after the failing
low-size cases: some later valid sizes (`0x1c`, `0x40`, `0x48`, or `0x4c`
depending on run) can return wait success and native `result_status=0` while
settings/output remain at their initialized values. Treat APUNN settings/output
changes, not host wait status alone, as the completion oracle. The kernel logs
show timeout/error lines for the rejected tiny code sizes and no oops or panic.
...
keeps opcode `XTENSA_ANN_VERSION`, one standard APUNN data descriptor, and the
settings-backed five-descriptor/no-settings request, then varies
`settings+0x08` / output header `+0x0c` across `0`, `4`, `0x10`, `0x3c`,
`0x40`, `0x44`, and `0x80`. It also tests output header `+0x10 = 1` for
sizes `0x40` and `0x44`. Controls preserve every initialized output header.
Dispatch plus wait returns `0` for every tested shape, changes settings
`0x5 -> 0x7`, and clears `settings+0x30` because data descriptor size remains
the standard `0x0c`. Small output sizes `0`, `4`, and `0x10` leave the existing
header words intact except for the already-initialized first `0xffffffff` word.
Larger sizes show that `settings+0x08` is an output-fill bound: `0x3c`,
`0x44`, and `0x80` reproducibly fill through their requested extent. The
`0x40` and header-flag cases can still partially skip across repeats, so the
field is a maximum writeback bound rather than a guarantee that every word is
...
`settings+0x0c` and whether `settings+0x30` points at a descriptor. Dispatch
plus wait returns `0` for all tested cases, changes settings `0x5 -> 0x7`, and
writes output through offset `0x40`. With no data pointer (`size=0`, pointer
zero), `settings+0x30` naturally remains zero. With a non-null pointer but
`data_desc_size=0`, firmware leaves `settings+0x30` and the descriptor entry
unchanged. With the standard one-entry size `0x0c`, firmware clears
`settings+0x30` and leaves the descriptor entry unchanged. With larger sizes
`0x18` and `0x80`, firmware leaves `settings+0x30` nonzero and clears data
descriptor entry word `0` from `3` to `0` while preserving the size and IOVA
words. This makes `0x0c` the current standard descriptor-consumption size for
the wrapper one-data shape; larger descriptor sections select a different
cleanup path without making output source-sensitive.
...
keeps the same completed settings-backed request and explicitly fills the
12-byte descriptor entries instead of leaving extra entries zeroed. Single-entry
cases with flags `1`, `2`, and `3`, plus a flags-`3` entry with size `0`, all
return `0` from dispatch and wait, change settings `0x5 -> 0x7`, clear
`settings+0x30`, leave the descriptor entry unchanged, and write output. Two
entry cases both complete and keep `settings+0x30` nonzero. When entry `0`
points at the APUNN data-payload window and entry `1` points at the plane window,
firmware clears the flags word of both entries while preserving their size/IOVA
words. When the order is reversed (`plane` then `payload`), both entries stay
unchanged. This makes the larger-descriptor cleanup target-dependent rather than
a pure `data_desc_size > 0x0c` side effect. The data and plane payload bytes
remain unchanged in every tested data-entry case.
...
keeps the same settings-backed request and points explicit descriptor entries at
each currently separated section: settings, code, output, data descriptor, data
payload, and plane payload. Single-entry cases for all six targets return `0`
from dispatch and wait, change settings `0x5 -> 0x7`, clear `settings+0x30`,
leave the descriptor entry unchanged, and leave the target payload bytes
unchanged. Two-entry order-pair cases across payload/settings, payload/code, and
payload/output also complete and keep `settings+0x30` nonzero; in these six
orderings firmware clears both entry flags while preserving each entry's
size/IOVA words. The earlier `plane_then_payload` data-entry case remains the
known exception where entries stayed unchanged, so plane-first cleanup should be
treated as a separate target/order behavior. The output remains
completion-style and no tested section target is copied into output.
...
keeps the standard data descriptor and varies the first data payload word base
across `0`, `0x41505530`, `0x5a5a0000`, and `0x7f000000`. Dispatch plus wait
returns `0` for every pattern, changes settings `0x5 -> 0x7`, clears
`settings+0x30`, leaves the data descriptor unchanged, and leaves the data
payload bytes unchanged. Output remains completion-style `0xffffffff` data and
does not reflect the chosen payload pattern. Repeats still show occasional
output-tail skips, but those skips do not correlate with payload contents. This
is the current strongest negative result against a source-sensitive writeback
leak in the tested completed `XTENSA_ANN_VERSION` shape.
...
the default normal-VPU opcode-4 provider clears this field before
`vpu_execute()`. The no-dispatch control preserves the requested value in the
command-buffer tail, while dispatch copyback clears tail word `40`
(`request+0xb68`) because the executed slot is kernel-selected. Dispatch plus
wait returns `0` for every submitted value and leaves settings/output/data
windows unchanged. The first dispatch run missed visible descriptor-0 writeback
for submitted values `1` and `2`, but the repeat wrote `0x2713 -> 0x271b` for
every tested value. Therefore the submitted word is not the missing APUNN
completion/output condition; the firmware slot input on this path is `0` unless
the explicit Preload/slot path writes an internal allocator slot.
...
|---|---|---|---|
| Firmware interaction | Completion path established | `system_app` can import HardwareBuffer dmabufs, submit full-size VPU requests, reach `apu_lib_apunn`, drive the D2D_EXT handoff, and now complete the wrapper-shaped `settings5/no-settings` request. The completed run uses five descriptors pointing at the DSP command/settings buffer, returns `0` from dispatch and wait, transitions settings flags `0x5 -> 0x7`, clears standard one-entry settings `+0x30`, leaves the code window unchanged, and writes the output section. The completed-shape opcode matrix shows `10001..10009` all complete. Completed-shape output operand ids `0/1/2/3/0xffff`, operand-list offsets `0/0x10/0x40/0x100/0x17e/0x180`, input/output count combinations `0/0`, `0/1`, `0/2`, `1/0`, `1/1`, and `2/1`, clean code sizes from `0x11` upward in the tested set, output sizes `0..0x80`, data descriptor sizes `0/0x0c/0x18/0x80`, single-entry flags `1/2/3`, size-0 entries, two-entry data/plane and payload/settings/code/output orderings, single-entry targets across settings/code/output/data-desc/data-payload/plane-payload, and tested data payload word patterns also complete. The `0x180` operand-list offset places the operand just outside the advertised operation entry and still completes under this oracle. Tiny code sizes `0/4/8/0xc/0x10` fail with `-EIO`; after those failing cases, later valid cases can return wait success without APUNN settings/output completion, so settings/output memory is the oracle. Earlier code-first, output-first, opcode, descriptor-size, priority, buffer-count, port-id, format, plane-count, height, operand-id, operand-offset, and op-shape matrices remain useful for parser and descriptor-binding attribution. | Exact higher-level APUNN output/data structure semantics for real NN operations, source-sensitive output behavior beyond `XTENSA_ANN_VERSION`, and service-wrapper lifetime/slot behavior. |
| Writeback leak | Not established | The completed `settings5/no-settings` run writes APUNN output data, and the no-dispatch control leaves that section unchanged. Completed-shape output-size tests show `settings+0x08` bounds the maximum output fill. Completed-shape data-payload tests with bases `0`, `0x41505530`, `0x5a5a0000`, and `0x7f000000` leave payload unchanged and do not change output content. The data-entry and data-target matrices leave data/plane payload bytes and targeted settings/code/output/data windows unchanged apart from descriptor flag cleanup and normal completion state. Earlier descriptor-backed deltas follow descriptor slot `0` and look like status/error words. | A source-sensitive copyback or leak from firmware/APUNN output memory into attacker-readable memory. |
...
1. **VPU D2D_EXT parameter ABI and IOVA-chain validation**: Highest current APUSYS task. `system_app` can import HardwareBuffer dmabufs, get APUSYS IOVA values, submit full-size normal-VPU requests, and reach `apu_lib_apunn`. IDA confirms the firmware handoff in `vpu_execute_d2d_handoff`: `request+0x50` is copied as `buffer_count * 0x40` bytes into the clamped-slot D2D buffer, then firmware receives `buffer_count`, the copied `struct vpu_buffer[]` IOVA, `sett_ptr`, and `sett_length` through `XTENSA_INFO12..15`; D2D_EXT also carries the kernel-selected slot plus preload entry/IRAM through `XTENSA_INFO11/16/19`. Runtime shows VPU boot/map activity, descriptor-following into imported memory, submitted `request+0xb68` values `0/1/2/3/0xffffffff` being rewritten by the default provider, and a clear `buffer_count` threshold where `0` fails but any tested nonzero value through `0x20` reaches the state-write path. The `settings5/no-settings` shape now proves APUNN settings/output completion for `10001..10009`, output operand ids `0/1/2/3/0xffff`, tested operand-list offsets through boundary/OOB placement, tested input/output count combinations, clean code sizes from `0x11` upward in the tested set, output sizes through `0x80`, standard and nonstandard data descriptor sizes, explicit data-entry flags/orderings, descriptor target sections, and tested data payload patterns; richer output/data structure semantics remain to map.
2. **Firmware completion/output contract under VPU dispatch**: The one-buffer internal query/status shapes timeout in the VPU worker. The two-native-buffer internal-command-shaped probe changes lifecycle behavior: `run_cmd_async` returns `0`, teardown logs residual command state, and the visible writeback follows copied native buffer descriptor `0`. Libvpu-style descriptor metadata (`port_id=1`, `format=0`, `plane_count=1`, `height=1`, `stride=size`), a five-descriptor alias shape, five identical code/input descriptors, five descriptor roles split as code/output/data-desc/data/plane, wrapper send-state settings `+0x00 = 0x5`, pre-seeded completion-like settings `+0x00 = 0x2/0x3`, output-first descriptor order, `request+0x38 = 0x68`, clearing the settings tuple to match the target wrapper's no-`setProperty` path, output header `+0x10 = 1`, wrapper-default output size `0x40`, wrapper dynamic output size `0x44`, one standard APUNN data descriptor, caller-supplied `request+0x28` bit `2`, relocated zero-output operand-list offsets `0/0x10/0x40/0x100`, output operand ids `0/1/2/3/0xffff`, input/output count combinations `0/0`, `0/1`, `0/2`, `1/0`, `1/1`, and `2/1`, descriptor-0 first-word values `0/0x2713/0x271b/0x504c4e30/0xffffffff`, descriptor size values `0..9/0xc/0x10/0x20/0x40/0x1c8`, submitted `request+0xb68` values `0/1/2/3/0xffffffff`, nonzero request buffer counts `1/2/3/4/5/0x20`, descriptor port ids `0/1/2/3/4/0xff`, descriptor format bytes `0/1/2/3/4/0xff`, descriptor plane counts `0/1/2/3/4/0xff`, and descriptor heights `0/1/2/3/0x40/0xffffffff` do not change the code-first status-write boundary. The code-first opcode matrix changes firmware behavior: `10005/10006/10007` timeout and `10006/10007` rewrite to `10005`. The fixed descriptor-layout matrix shows code-first and mixed code-first layouts wait successfully and write only code word `0`, while output-first moves the write to output word `0` and returns `-EIO`. Static wrapper analysis now separates those diagnostic property/code-first probes from the ordinary target `CreateVpuRequest()` path. The completed `settings5/no-settings` matrices point all five native descriptors at the DSP command/settings buffer; completed cases change settings flags to the wrapper completion predicate and leave the code window unchanged. `settings+0x04` gates code-section acceptance: tiny sizes `0/4/8/0xc/0x10` fail, `0x11` is the smallest tested completed size, and clean/success-only runs complete every selected valid size through `0x1c9`. Full range runs show a batch-state caveat after failures: wait success and native `result_status=0` can appear without APUNN settings/output completion. The standard one-data size `0x0c` clears `settings+0x30`; single-entry flags `1/2/3`, entry size `0`, and single-entry targets across settings/code/output/data-desc/data-payload/plane-payload still complete; two-entry sections keep `settings+0x30` nonzero; and flags cleanup is target/order-sensitive. Tested payload contents do not flow into output. The remaining firmware work is source-sensitive behavior for real operation/data bindings, not basic completion reachability.
...
- Map how `apu_lib_apunn` uses the copied `struct vpu_buffer[]` beyond the completed `settings5/no-settings` shape. The outer APUSYS `cb_info_size` gate is exact `0xb70`; non-exact sizes fail before descriptor writeback. Earlier matrices rule out ordinary libvpu metadata, code-first aliases, output-first layouts, settings-property use, output header/size tweaks, data-descriptor presence, preload bit selection, operand-list offsets, output operand ids, input/output count combinations, and descriptor metadata as standalone completion triggers. The positive completion condition observed so far is five native descriptors pointing at the DSP command/settings buffer with a sufficiently large code section; in that shape, opcodes `10001..10009`, tested output operand ids, tested operand-list offsets including `0x180` OOB placement, tested input/output count combinations, clean code sizes from `0x11` upward in the tested set, tested output sizes, tested data descriptor sizes, tested explicit data-entry flags/orderings, tested descriptor targets, and tested data payload patterns all complete. The full range runs add the caveat that wait success/native `result_status=0` after low-size failures is not enough; settings/output mutation remains the completion oracle.
- Map the completed-shape output/data structure semantics for real operation bindings. The current positive runs change settings flags to satisfy `(settings[0] & 0x0a) == 0x02`, leave the code window unchanged, and write output across opcode, operand-id, operation-count, code-size, output-size, data-descriptor, data-entry, data-target, and data-payload matrices. `settings+0x04` is the observed code-section acceptance gate, `settings+0x08` bounds output fill, standard `data_desc_size=0x0c` clears `settings+0x30`, two-entry descriptors preserve `settings+0x30` with target/order-sensitive flags cleanup, and tested payload contents do not flow into output. The remaining question is how wrapper-generated command-buffer ids, real input/output buffer bindings, and nontrivial APUNN operations affect those completed output bytes.
- Shift the next matrix toward the standard wrapper path recovered from `libneuron_platform.so`: command-buffer id input binding, `PrepareXtensaCommandBuffer()`, output allocation/binding, `PrepareOutputBuffer()`, and `PrepareDataBuffer()` / `FinalizeDataBuffer()`. The direct ioctl replay now gives a stable completion oracle for comparing those wrapper-generated structures.
- Attribute the completed output writeback semantically: the current output words look deterministic or partially skipped rather than source-sensitive, so leak testing should move from synthetic payload patterns to wrapper-generated source buffers and real operation/data bindings.
