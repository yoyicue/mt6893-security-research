...
The current allocator-control target is documented in [`ALLOCATOR_CONTROLLABILITY_OPPORTUNITY.md`](ALLOCATOR_CONTROLLABILITY_OPPORTUNITY.md). It tracks the `target_then_lower` exact IOVA reuse opportunity, the metrics required to make that opportunity controllable, and the firmware re-entry criteria.
