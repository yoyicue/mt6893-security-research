...
The completed-shape code-size matrix
`--run-cmd-vpu-xrp-target-settings5-no-settings-code-size-matrix-iova` keeps
opcode `XTENSA_ANN_VERSION`, one standard APUNN data descriptor, output size
`0x40`, and the settings-backed five-descriptor/no-settings request, then varies
`settings+0x04` across `0`, `4`, `0x48`, `0x1c7`, `0x1c8`, `0x1c9`, and
`0x390`. Its control preserves every initialized code-size value and skips only
dispatch. Dispatch plus wait fails for code sizes `0` and `4` with
`mdw_usr_wait_cmd = -EIO`, leaves settings at `0x5`, preserves
`settings+0x30`, keeps the output header unchanged, and sets native request
`result_status=0x2`. Code size `0x48` is the smallest tested value that reaches
the normal completion predicate: settings become `0x7`, `settings+0x30` is
cleared, and output is filled through offset `0x40`. Code sizes `0x1c7`,
`0x1c8`, and `0x390` complete in both dispatch runs. Code size `0x1c9` completed
once, then repeated with wait success and native `result_status=0` but without
APUNN settings/output completion; treat it as an unstable or alignment-sensitive
edge until isolated. The kernel logs show timeout/error lines for the tiny code
sizes and no oops or panic.
