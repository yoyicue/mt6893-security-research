...
|---|---|---|---|
| Firmware interaction | Partial, enough for parser-oriented experiments | `system_app` can import HardwareBuffer dmabufs, submit full-size VPU requests, reach `apu_lib_apunn`, and drive the D2D_EXT handoff with firmware-visible descriptor/settings IOVAs. Host debug helpers also pin the basic Xtensa operation fields and confirm `10003` as `XTENSA_ANN_VERSION`. | Normal APUNN completion/output contract: `XTENSA_INFO00/02`, settings `(flags & 0x0a) == 0x02`, and wrapper `WritebackCommand()` output flow. |
