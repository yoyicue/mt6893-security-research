...
|---|---|---|---|
| Firmware interaction | Completion path established | `system_app` can import HardwareBuffer dmabufs, submit full-size VPU requests, reach `apu_lib_apunn`, drive the D2D_EXT handoff, and now complete the wrapper-shaped `settings5/no-settings` request. The completed run uses five descriptors pointing at the DSP command/settings buffer, returns `0` from dispatch and wait, transitions settings flags `0x5 -> 0x7`, clears settings `+0x30`, leaves the code window unchanged, and writes the output section. Earlier code-first, output-first, opcode, descriptor-size, priority, buffer-count, port-id, format, plane-count, height, operand-id, and op-shape matrices remain useful for parser and descriptor-binding attribution. | Exact higher-level APUNN opcode/output semantics beyond the completed `XTENSA_ANN_VERSION` shape, plus service-wrapper lifetime/slot behavior. |
