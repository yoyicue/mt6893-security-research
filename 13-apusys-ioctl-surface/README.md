...
|---|---|---|---|
| Firmware interaction | Completion path established | `system_app` can import HardwareBuffer dmabufs, submit full-size VPU requests, reach `apu_lib_apunn`, drive the D2D_EXT handoff, and now complete the wrapper-shaped `settings5/no-settings` request. The completed run uses five descriptors pointing at the DSP command/settings buffer, returns `0` from dispatch and wait, transitions settings flags `0x5 -> 0x7`, clears settings `+0x30`, leaves the code window unchanged, and writes the output section. The completed-shape opcode matrix shows `10001..10009` all complete; `10004` differs only by writing `0x40` bytes of output instead of the usual `0x44`. Earlier code-first, output-first, opcode, descriptor-size, priority, buffer-count, port-id, format, plane-count, height, operand-id, and op-shape matrices remain useful for parser and descriptor-binding attribution. | Exact higher-level APUNN output/data structure semantics, source-sensitive output behavior, and service-wrapper lifetime/slot behavior. |
| Writeback leak | Not established | The completed `settings5/no-settings` run writes deterministic APUNN output data, and the no-dispatch control leaves that section unchanged. Earlier descriptor-backed deltas follow descriptor slot `0` and look like status/error words. | A source-sensitive copyback or leak from firmware/APUNN output memory into attacker-readable memory. |
...
- Attribute the completed output writeback semantically: the current output words look deterministic, so leak testing should vary source buffers, output lengths, operand ids, and data-descriptor contents in the completed settings-backed shape and look for source-sensitive bytes.
