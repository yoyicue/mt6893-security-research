...
|---|---|---|---|
| 1 | In-flight `mem_free(shared_iova)` + exact IOVA reuse + replacement buffer | Highest current APUSYS opportunity, but firmware timing analysis (2026-06-15) shows the race window for `XTENSA_ANN_VERSION` is sub-Java-latency | Allocator half confirmed (64K `p16/r8 target_then_lower`, 5% hit rate). Firmware byte analysis of `0x70044b74` shows INPUT DMA is synchronous (schedule→wait in consecutive FLIX128 bundles, 9-byte gap) and OUTPUT write is likely CPU-store inside `0x700a21b8` (20 KB compute kernel). Both paths complete before VPU signals done. Re-entry gate (`exact_iter/adjacent ≥ 10%`) not yet met. The only untested path that could widen the window is a slower opcode triggering a different branch inside `0x700a21b8` (the `ann_output40` 14 ms anomaly is unexplained and may indicate such a branch). |
| 2 | Descriptor-plane relationship write redirect | Strong firmware-control lead and currently the **only identified slow-write path** (≈9 s timeout); not yet combined with IOVA reuse | Runtime fuzz of descriptor `+0x20/+0x24/+0x28/+0x34/+0x38..+0x3b` can redirect a firmware-visible write into the imported plane window, changing `PLN0 -> PLN1`. The write is still confined to the caller-imported IOVA. The plane-redirect path ends in ~9 s timeout/EIO, far slower than the completed path. This makes it the strongest candidate for a `mem_free` race that can be won from Java, if combined with exact IOVA reuse. Untested combination: plane-redirect + `target_then_lower` gap reuse. |
...
|---|---|---|
| Firmware/provider writes after `mem_free` | Explicit `mem_free` during in-flight commands is reachable from `system_app`; timeout cases return clean `-EIO` and completed cases usually finish normally | **Firmware byte analysis (2026-06-15) shows INPUT DMA is synchronous (schedule→wait in consecutive FLIX128 bundles, 9-byte gap) and OUTPUT write is likely CPU-store inside 20 KB compute kernel `0x700a21b8`. Both complete before VPU signals done. The `free_after=0ms` completed case shows firmware wins every time at Java latency.** |
...
| Descriptor-plane write can be combined with replacement reuse | Descriptor-plane fuzz can redirect a firmware-visible write into the imported plane window (`PLN0 -> PLN1`) | The redirect is currently confined to the original imported IOVA; it is untested as a post-free write into an exact-reused replacement. **The plane-redirect path produces ~9s timeout, far slower than completed path. This is currently the only identified firmware write path slower than the Java mem_free round-trip.** |
...
The probe now includes the next highest-risk timing variants in the same mode:
the original `wrapper_one_data` output-size `0x40` baseline, an
`output_size=0x1000` completed shape, and an `output_size=0x1000` case with a
`250 us` post-`run_cmd_async` delay before freeing the target/lower IOVAs. The
goal is to test the narrow window between APUNN consuming valid settings and
finishing the bounded output writeback.
...
4K p12/r20, output_size=0x40, free_delay_us=0:
  pair_found=3/40, exact_target=0/60, completion_like_hits=0, wait_ok=3, wait_eio=0

4K p16/r16, output_size=0x1000, free_delay_us=0:
  pair_found=7/30, exact_target=0/112, completion_like_hits=0, wait_ok=6, wait_eio=1

4K p16/r16, output_size=0x1000, free_delay_us=250:
  pair_found=5/30, exact_target=0/80, completion_like_hits=0, wait_ok=5, wait_eio=0
```

This run did not reproduce an exact target IOVA hit, so the larger
`settings+0x08` output window and `250 us` delay did not produce a replacement
write. Kernel filtering found one timeout-class `mdw_sched_trace ret(-110)` and
