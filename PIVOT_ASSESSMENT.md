...
**Risk**: Medium-high research priority, with a working framework fd source for memory import and runtime-confirmed `ucmd` / `run_cmd_async` parser reachability. The dispatcher uses fixed `copy_from_user` sizes and several early validation gates, but `app_process64` can create a HardwareBuffer dmabuf, both APUSYS type-2/type-3 memory-create paths import it successfully, and changing the mapped buffer's first u32 from `0` to `0x8001` changes normal VPU `ucmd` from `EINVAL` to `ENOENT`. Static analysis maps that `ENOENT` to the Normal/Preload lookup-miss path at `0xffffffc0087a0c70`; the empty-list path returns success instead. Userspace `/vendor/lib64/libvpu.so` confirms the lookup payload shape: `VpuStreamImp::getAlgo()` sends `0x8001` plus up to 31 key bytes at payload offset `+4`. The `--ucmd-key=Normal` probe uses that shape and still returns `ENOENT`, so `Normal` is not a loaded algorithm key. The command ops table is now resolved through kallsyms-derived raw-pointer normalization: `mdw_usr_run_cmd_async` calls `mdw_cmd_create_cmd`, then `mdw_cmd_parse_cmd`, and success queues a handle back to user `+0x00`. The zero-header HardwareBuffer command-buffer probe imports successfully and returns `EINVAL` from the parser path before queue insertion. The most interesting paths are now valid command-buffer layout, actual VPU algorithm keys, and provider dispatch reached after memory mapping.

**Action**: Continue from [`13-apusys-ioctl-surface/README.md`](13-apusys-ioctl-surface/README.md). Recover actual Normal/Preload VPU algorithm keys and the APUSYS command-buffer layout before running positive-key or valid-command experiments.
