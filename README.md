...
| [13](13-apusys-ioctl-surface/) | APUSYS ioctl surface | `/dev/apusys` opens from `system_app`; provider opcode-0 dispatch is live, HardwareBuffer under `app_process64` supplies an importable dmabuf, and normal VPU `ucmd` reaches beyond the `0x8001` mapped-buffer gate to `ENOENT`. |
