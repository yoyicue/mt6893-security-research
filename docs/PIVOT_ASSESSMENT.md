...
**Runtime result**: `../poc-run-results/2026-06-14-batch/13_apusys_mem_ion.txt` uses the old ION ABI observed in IDA (`ALLOC 0xc0204900`, `SHARE 0xc0084904`, `FREE 0xc0044901`) and fails before ioctl because `/dev/ion` cannot be opened from `system_app`.
