...
| `13_apusys_run_cmd_invalid_sc.txt` | APUSYS `run_cmd_async` valid top-header / invalid-subcommand parser check |
| `13_apusys_run_cmd_invalid_sc_kernel.txt` | Kernel log proof that the invalid-subcommand probe reaches `mdw_cmd_sc_valid invalid type(32)` |
| `13_apusys_run_cmd_vpu_guard.txt` | APUSYS `run_cmd_async` valid normal-VPU subcommand / short request-size guard probe |
| `13_apusys_run_cmd_vpu_guard_kernel.txt` | Kernel log proof that the valid-type probe reaches `vpu_req_check: invalid size of vpu request` |
| `13_apusys_run_cmd_vpu_iova_final.txt` | APUSYS corrected full-size normal-VPU IOVA dispatch with original-buffer post-run dump |
| `13_apusys_run_cmd_vpu_iova_final_kernel.txt` | Kernel log from the corrected IOVA dispatch run |
| `13_apusys_run_cmd_vpu_iova_control.txt` | Same IOVA import setup with `run_cmd_async` skipped to isolate buffer writeback |
| `13_apusys_run_cmd_vpu_iova_control_kernel.txt` | Kernel log from the no-dispatch IOVA control run |
| `13_apusys_run_cmd_vpu_xrp_iova.txt` | XRP-shaped APUNN settings dispatch; data payload first word changes while settings/output/descriptor and command-buffer windows stay unchanged |
| `13_apusys_run_cmd_vpu_xrp_iova_kernel.txt` | Kernel log from the XRP-shaped APUNN dispatch run |
| `13_apusys_run_cmd_vpu_xrp_iova_control.txt` | Same XRP-shaped imported-buffer setup with `run_cmd_async` skipped |
| `13_apusys_run_cmd_vpu_xrp_iova_control_kernel.txt` | Kernel log from the XRP-shaped no-dispatch control run |
| `13_apusys_run_cmd_vpu_xrp_split_iova.txt` | XRP-shaped split-target dispatch; APUNN data payload stays unchanged while native VPU plane0 MVA target first word changes |
| `13_apusys_run_cmd_vpu_xrp_split_iova_kernel.txt` | Kernel log from the XRP-shaped split-target dispatch run |
| `13_apusys_run_cmd_vpu_xrp_split_iova_control.txt` | Same split-target setup with `run_cmd_async` skipped |
| `13_apusys_run_cmd_vpu_xrp_split_iova_control_kernel.txt` | Kernel log from the split-target no-dispatch control run |
| `13_apusys_run_cmd_vpu_xrp_ann_version_iova.txt` | Nonzero `code_size=0x1c8` APUNN/XRP dispatch using opcode `10003`; output/data windows stay unchanged while native VPU plane0 MVA target first word changes |
| `13_apusys_run_cmd_vpu_xrp_ann_version_iova_kernel.txt` | Kernel log from the nonzero APUNN/XRP dispatch run |
| `13_apusys_run_cmd_vpu_xrp_ann_version_iova_control.txt` | Same nonzero APUNN/XRP setup with `run_cmd_async` skipped |
| `13_apusys_run_cmd_vpu_xrp_ann_version_iova_control_kernel.txt` | Kernel log from the nonzero no-dispatch control run |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc.txt` | Internal `XTENSA_ANN_VERSION` two-buffer dispatch with libvpu-style descriptor metadata; code/input first word changes, output/data windows remain unchanged |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc_kernel.txt` | Kernel log from the libvpu-style descriptor dispatch run |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc_control.txt` | Same libvpu-style descriptor setup with `run_cmd_async` skipped |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc_control_kernel.txt` | Kernel log from the libvpu-style descriptor no-dispatch control |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc_send_flags.txt` | Same two-buffer/libvpu descriptor dispatch with wrapper send-state settings `+0x00 = 0x5`; settings remain `0x5`, code/input changes, output/data remain unchanged |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc_send_flags_kernel.txt` | Kernel log from the wrapper send-state dispatch run |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc_send_flags_control.txt` | Same wrapper send-state setup with `run_cmd_async` skipped |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc_send_flags_control_kernel.txt` | Kernel log from the wrapper send-state no-dispatch control |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc5.txt` | Internal `XTENSA_ANN_VERSION` dispatch with libvpu-style metadata and five copied descriptors; code/input first word changes, output/data windows remain unchanged |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc5_kernel.txt` | Kernel log from the five-descriptor dispatch run |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc5_control.txt` | Same five-descriptor setup with `run_cmd_async` skipped |
| `13_apusys_run_cmd_vpu_xrp_internal_ann_version_iova_libvpu_desc5_control_kernel.txt` | Kernel log from the five-descriptor no-dispatch control |
...
- APUSYS `mdw_usr_run_cmd_async` command ops are now resolved through kallsyms-derived raw-pointer normalization. With the same HardwareBuffer fd source, a zero command buffer reaches memory import and returns `EINVAL` from the parser path before queue insertion. Follow-up IDA work recovers the top-level command header magic/version/count/offset gates, the 0x28-byte subcommand header, the queued scheduler handoff, `mdw_cmd_sc_set_hnd` codebuf copy, provider opcode-4 dispatch through `provider_desc+0x18`, and the normal VPU opcode-4 request checks. Runtime confirms deeper stages: invalid subcommand type reaches `mdw_cmd_sc_valid invalid type(32)`, a short normal VPU subcommand reaches `vpu_req_check: invalid size of vpu request`, full-size `0xb70` normal VPU requests are accepted asynchronously, and the corrected IOVA request reaches VPU boot/map activity. The initial minimal IOVA dispatch changed the imported buffer first word from `0` to `0xb`; the same-target XRP-shaped APUNN settings run narrowed the visible effect to the shared APUNN-data/native-plane target; the split-target XRP-shaped run localizes it further to native VPU plane0 MVA, changing first word from `0x504c4e30` to `0x504c4e31` while settings, output, data descriptor, APUNN data payload, and command-buffer copyback windows stay unchanged. A nonzero `code_size=0x1c8` / opcode `10003` XRP-shaped run is accepted at the dispatch level and shows the same plane0-MVA-only delta while APUNN output/data windows remain unchanged. A host-wrapper-derived two-native-buffer `XTENSA_ANN_VERSION` run changes lifecycle behavior: `run_cmd_async` returns `0`, kernel logs show VPU map/boot activity without the earlier `D2D_EXT timeout`, teardown logs `mdw_usr_destroy residual cmd`, and the visible writeback moves to native buffer `0` at the code/input window (`0x2713 -> 0x271b`) while APUNN output/data windows remain unchanged. Libvpu-style descriptor metadata (`port_id=1`, `height=1`, `stride=size`), a five-descriptor alias shape, and the wrapper send-state command flag value `0x5` produce the same code/input-only writeback without APUNN output/data completion. The matching no-dispatch controls keep the same buffers unchanged.
...
|---|---|---|
| APUSYS CVE candidates | High research priority | `/dev/apusys` opens with `O_RDWR`; `0x400C4109` provider opcode-0 dispatch is live for MDLA, normal VPU, EDMA, and MDLA RT. HardwareBuffer under `app_process64` provides a usable dmabuf fd, both APUSYS memory-create variants import it successfully, normal VPU `ucmd` reaches the `0x8001 + key` lookup path, `apu_lib_apunn` returns provider success, and `run_cmd_async` reaches parser, queue, scheduler, full-size normal-VPU request dispatch, VPU boot/map-side activity, a split-target native VPU plane writeback, a tolerated nonzero `0x1c8` APUNN/XRP code section, and a two-native-buffer internal-command shape that changes the timeout into residual command state. Libvpu-style descriptor metadata, `buffer_count=5` aliases, and wrapper send-state command flags do not produce APUNN output-section/data-descriptor consumption, which remains open. |
...
2. **APUSYS reachable surface**: APUSYS-related CVEs should be mapped next because `/dev/apusys` opens from `system_app`, provider dispatch is live, HardwareBuffer under `app_process64` supplies a dmabuf fd that APUSYS imports successfully, normal VPU `ucmd` reaches the userspace-compatible `0x8001 + key` lookup path, `apu_lib_apunn` returns provider success, and `run_cmd_async` reaches parser, queue, scheduler, full-size normal-VPU request dispatch, VPU boot/map-side activity, a reproducible native VPU buffer writeback, a tolerated nonzero `0x1c8` APUNN/XRP code section, and a two-native-buffer internal-command shape that changes the timeout into residual command state. Libvpu descriptor metadata/count variants and wrapper send-state flags do not complete APUNN output. The next APUSYS step is internal input-buffer contents, output header/status semantics, and the firmware completion predicate, not basic ioctl reachability.
