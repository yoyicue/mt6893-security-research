...
2. APUSYS: recover the request and command-buffer layouts after the HardwareBuffer-backed `0x8001 + key` lookup and zero-header `run_cmd_async` parser gates; the `apu_lib_apunn` lookup success path is currently mapped as `vpu_alg_get` / `vpu_alg_put` with no visible first-64-byte writeback.
