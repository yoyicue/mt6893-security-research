...
2. APUSYS: continue from the corrected VPU IOVA dispatch. The top-level command header, 0x28-byte subcommand header, queued scheduler handoff, `mdw_cmd_sc_set_hnd` codebuf copy, provider opcode-4 call, normal VPU request-size guard, full-size request acceptance, VPU boot/map-side activity, and small imported-buffer writeback are now runtime-confirmed. The next target is `apu_lib_apunn` settings-payload semantics and writeback attribution, plus separate MDLA/EDMA request mapping.
