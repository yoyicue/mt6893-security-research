...
| APUSYS ioctl surface | High research priority | `/dev/apusys` opens from `system_app`; provider opcode-0 dispatch, memory-create, normal VPU opcode-7 `ucmd`, and `run_cmd_async` parser reachability are mapped. Direct node fd sources are constrained, but `app_process64` can create a HardwareBuffer dmabuf, both APUSYS type-2/type-3 memory-create paths import it successfully, normal VPU `ucmd` reaches the userspace-compatible `0x8001 + key` lookup path, `apu_lib_apunn` returns provider success on core `0` and `1` without visible first-64-byte Image-plane writeback, and zero-header `run_cmd_async` reaches parser rejection (`EINVAL`). Runtime confirms the recovered command/subcommand header gates by reaching `mdw_cmd_sc_valid invalid type(32)`, then confirms the queued provider handoff with a valid normal VPU `type=0x03` subcommand that reaches `vpu_req_check: invalid size of vpu request` in `apusys_vpu0`. Static recovery maps the handoff: `mdw_sched_dev_routine` calls `mdw_cmd_sc_set_hnd`, copies the parsed codebuf into a provider argument buffer, invokes `provider_desc+0x18` with opcode `4`, then `mdw_cmd_sc_clr_hnd` copies the temporary buffer back to the source KVA and frees it. |
...
2. APUSYS: continue from the non-executing valid-type probe. The top-level command header, 0x28-byte subcommand header, queued scheduler handoff, `mdw_cmd_sc_set_hnd` codebuf copy, provider opcode-4 call, and normal VPU request-size guard are now runtime-confirmed. The next target is request-structure mapping before any real VPU/MDLA/EDMA work submission.
