...
2. APUSYS: continue provider-specific code-buffer and request mapping after the HardwareBuffer-backed `0x8001 + key` lookup and zero-header `run_cmd_async` parser gates; the top-level command header, 0x28-byte subcommand header, and normal VPU opcode-4 request checks are partially recovered, and the `apu_lib_apunn` lookup success path is mapped as `vpu_alg_get` / `vpu_alg_put` with no visible first-64-byte writeback.
