...
2. APUSYS: build the next non-executing valid-type probe after the HardwareBuffer-backed `0x8001 + key` lookup and `run_cmd_async` parser gates; the top-level command header, 0x28-byte subcommand header, queued scheduler handoff, `mdw_cmd_sc_set_hnd` codebuf copy, and provider opcode-4 call are statically mapped. The next target is a deterministic provider request guard, not real VPU/MDLA/EDMA work submission.
